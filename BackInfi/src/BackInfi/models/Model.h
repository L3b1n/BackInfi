...
class Model {
  ...
  {
    char *modelFilepath_rawPtr = (char *)modelSelection.c_str();

    if (modelFilepath_rawPtr == nullptr) {
      std::cerr << "Error! Unable to get model filename " << modelSelection.c_str() << " from plugin.\n";
      return nullptr;
    }

    std::string modelFilepath_s(modelFilepath_rawPtr);
    free(modelFilepath_rawPtr);
  ...
  */
                                   ...
                                   std::vector<std::vector<float>> &outputTensorValues)
  {
    ...
    //std::vector<float> predictions = outputTensorValues[0];
    ...
    //    float sum = 0.0;
    //    for (int s = 0; s < 2; ++s)
    //        if (max < predictions[i * 2 + s])
    //            max = predictions[i * 2 + s];
    //    for (int s = 0; s < 2; ++s)
    //    {
    //        predictions[i * 2 + s] = exp(predictions[i * 2 + s] - max);    // prevent data overflow
    //        sum += predictions[i * 2 + s];
    //    }
    //    for (int s = 0; s < 2; ++s) predictions[i * 2 + s] /= sum;
    //}

    //cv::Mat mask = cv::Mat(outputHeight, outputWidth, CV_32FC1);
    //for (int rowId = 0; rowId < outputHeight; ++rowId) {
    //    for (int colId = 0; colId < outputWidth; ++colId) {
    //        float pre = predictions[(rowId * outputWidth + colId) * 2 + 1];
    //        mask.at<float>(rowId, colId) = pre;
    //    }
    //}
    //return mask;
  }
                                   ...
                                   std::vector<Ort::Value> &outputTensor)
  {
    if (inputNames.size() == 0 || outputNames.size() == 0 || inputTensor.size() == 0 ||
        outputTensor.size() == 0) {
      std::cerr << "Error! Skip network inference. Inputs or outputs are null.\n";
      return;
    }
